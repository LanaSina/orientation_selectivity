...
configuration = "filter_prediction/contrast/OSWALD_20FPS/t2"
filterFolder = "x_159_y_253/"
direction = "neuron_x"
...
#h or v
d = "v"
configuration = "filter_prediction/contrast/KITTI/t1" #OSWALD_20FPS
...
filename = paste(folderName, configuration, subfolder, "filters.pdf", sep = "")
print(filename)
...
ggplot(allProbas, aes(distance)) +
       ...
       geom_ribbon(aes(ymin = random - random_sd,
                       ymax = random + random_sd), alpha = 0.2) +
       geom_line(aes(y = full, color = "Full")) +
       geom_ribbon(aes(ymin = full - full_sd,
                       ymax = full + full_sd), alpha = 0.2) +
       labs(x="Distance", y="Predictive power", colour = "Filter type") +
       xlim(0, 50) + ylim(0,1)
